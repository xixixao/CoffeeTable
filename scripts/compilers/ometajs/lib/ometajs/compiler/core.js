define([
  ...
  return function (ometajs) {

  var exports = {};

var utils = ometajs.utils;
...
Compiler.prototype.addGrammars = function addGrammars(root) {
  this.enter('code', function() {
    ...
    // this is normal way to load ometajs object
    //this.insert('var _ometajs = require(', JSON.stringify(root), ');');

    //this is to get the interactive session working, require OmetaJS global
    this.insert('var _ometajs = window.ometajs;');

    var keys = Object.keys(ometajs.grammars);
    for (var i = 0; i < keys.length; i++) {
      this.insert('var ', keys[i], ' = _ometajs.grammars.', keys[i], ';');
    }
...
Compiler.prototype.compileExpression = function compileExpression(exp,
  ...
  switch(exp[0]) {
    ...
    case 're':
      ...
      break;
    case 'match':
      if (exp[1] !== null) throw new Error('match: Not implemented yet');
      this.insert(['rule', exp[2]]);
      break;
    case 'super':
      var match = exp[1];
      if (match[1] !== null) throw new Error('super: Not implemented yet');

      this.insert(['super', match[2]]);
      break;
    case 'call':
      if (exp[1] !== null) throw new Error('call: Not implemented yet');
